...
export const Modal = ({ children, addClass, isOpen, onClose, finalFocusRef, label, ...props }) => {
   ...
   return (
      <Portal id="js-modal">
         <div className={css["c-layout"]} onClick={onCloseModal} hidden={!isOpen}></div>
         <div
            role="dialog"
            tabIndex="-1"
            hidden={!isOpen}
         ...
         >
            ...
            <Button addClass={css["c-close-button"]} label="Cerrar modal" hasAriaLabel={true} icon={{ name: "close" }} onClick={onCloseModal} />
         </div>
      </Portal>
   );
};
